...
import Adivination from "../components/Adivination";
...
describe("Legislative Session", () => {
  ...
  function wrap() {
    return mount(
      <Provider store={store}>
        <Adivination hand={["nox", "lumos", "nox"]} />
      </Provider>
    );
  }
  ...
  it("should show button if modal opens", () => {
    const wrapper = wrap();
    const button = wrapper.find("div button");
    button.simulate("click");
    const modalButton = wrapper.find("div Modal div button");
