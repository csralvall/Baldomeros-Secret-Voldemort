...
function Match( {match} ) {
  ...
  const Election = (
    <div>
      ...
      { Object.entries(gameStatus.players).map(player =>(
        <h4>{player[0]} voted {player[1]}</h4>
      ))
      }
      <div> { gameStatus.players[user.username] === 'missing vote' ? (<Vote/>) : ""}</div>
    </div>
  )
  ...
  const Board = (
    <div>
      ...
      <h1> Death Eaters Proclamations : {gameStatus.deatheaterproclamations}/6 </h1>
    </div>
  )
  ...
  const Winner = (
    <div>
      <h1> 
        ...
        "Deaht Eaters" : "Phoenix Order"} team won the match
      </h1>
    </div>
  )
